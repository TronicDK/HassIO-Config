...
<script type="text/javascript">RED.nodes.registerType("ha-entity",{category:"home_assistant",color:"#52C0F2",inputs:1,outputs:1,icon:"font-awesome/fa-genderless",align:"right",paletteLabel:"sensor",label:function(){return this.name||`type: ${this.entityType}`},labelStyle:nodeVersion.labelStyle,defaults:{name:{value:""},server:{value:"",type:"server",required:!0},version:{value:1},debugenabled:{value:!1},outputs:{value:1},entityType:{value:"sensor"},config:{value:[{property:"name",value:""},{property:"device_class",value:""},{property:"icon",value:""},{property:"unit_of_measurement",value:""}]},state:{value:"payload"},stateType:{value:"msg"},attributes:{value:[]},resend:{value:!0},outputLocation:{value:"payload"},outputLocationType:{value:"none"},inputOverride:{value:"allow"}},oneditprepare:function(){nodeVersion.check(this);const e={sensor:["msg","flow","global","jsonata","str","num","bool","date"],binary_sensor:["msg","flow","global","jsonata","str","num","bool"]},t=["str","num","bool","date","jsonata","msg","flow","global"];haServer.init(this,"#node-input-server"),exposeNode.init(this),$("#node-input-state").typedInput({types:e[$("#node-input-entityType").val()],typeField:"#node-input-stateType",type:this.stateType}),$("#node-input-entityType").on("change",(function(){$("#node-input-state").typedInput("types",e[$(this).val()])})).trigger("change"),$("#attributes").editableList({removable:!0,addButton:"add attribute",header:$("<div>").append($.parseHTML("<div style='width:40%; display: inline-grid'>Attribute Key</div><div style='display: inline-grid'>Value</div>")),addItem:function(e,a,n){const i=$("<div />").appendTo(e);$("<input />",{type:"text",name:"property",style:"width: 40%;margin-right: 5px;"}).attr("autocomplete","disable").appendTo(i).val(n.property),$("<input />",{type:"text",name:"value",style:"width: 55%;"}).appendTo(i).val(n.value).typedInput({types:t}).typedInput("type",n.valueType)}}).editableList("addItems",this.attributes),$("#config").editableList({addButton:!1,header:$("<div>Home Assistant Config (optional)</div>"),addItem:function(e,t,a){const n=$("<div />").appendTo(e);$("<input />",{type:"text",name:"property",value:a.property,style:"width: 40%;",readonly:!0}).appendTo(n),$("<input />",{type:"text",name:"value",value:a.value,style:"margin-left: 10px;width: 55%;"}).attr("autocomplete","disable").appendTo(n)}}).editableList("addItems",this.config),$("#node-input-outputLocation").typedInput({types:["msg","flow","global",{value:"none",label:"None",hasValue:!1}],typeField:"#node-input-outputLocationType"})},oneditsave:function(){const e=this;e.attributes=[],e.config=[],nodeVersion.update(this),$("#attributes").editableList("items").each((function(t){const a=$(this);e.attributes.push({property:a.find("input[name=property]").val(),value:a.find("input[name=value]").typedInput("value"),valueType:a.find("input[name=value]").typedInput("type")})})),$("#config").editableList("items").each((function(t){const a=$(this);e.config.push({property:a.find("input[name=property]").val(),value:a.find("input[name=value]").val()})}))}});</script>
<script type="text/x-red" data-template-name="ha-entity"><input type="hidden" id="node-input-version"><div class="form-row"><label for="node-input-name">Name</label> <input type="text" id="node-input-name" placeholder="Name"> <input type="hidden" id="node-input-outputs"></div><div class="form-row"><label for="node-input-server">Server</label> <input type="text" id="node-input-server"></div><div class="form-row"><label for="node-input-entityType">Type</label> <select id="node-input-entityType" style="width:70%"><option value="sensor">Sensor</option><option value="binary_sensor">Binary Sensor</option></select></div><div class="form-row"><label for="node-input-state">State</label> <input type="text" id="node-input-state" style="width:68%"> <input type="hidden" id="node-input-stateType"></div><div class="form-row"><ol id="attributes"></ol></div><div class="form-row"><ol id="config"></ol></div><div class="form-row"><label for="node-input-outputLocation">Output Location</label> <input type="hidden" id="node-input-outputLocationType"> <input type="text" id="node-input-outputLocation" style="width:68%"></div><div class="form-row"><label for="node-input-inputOverride">Input Override</label> <select id="node-input-inputOverride" style="width:68%"><option value="allow">Allow</option><option value="merge">Merge</option><option value="block">Block</option></select></div><div class="form-row checkbox-option"><input type="checkbox" id="node-input-resend"> <label for="node-input-resend">Resend state and attributes</label></div><div class="form-row checkbox-option"><input type="checkbox" id="node-input-debugenabled"> <label for="node-input-debugenabled">Show Debug Information</label></div></script>
<script type="text/x-red" data-help-name="ha-entity"><p>Creates a sensor or binary sensor in Home Assistant which can be updated from this node.</p><div class="custom-block warning"><p class="custom-block-title">WARNING</p><p><em>Needs <a href="https://github.com/zachowj/hass-node-red">Custom Integration</a> installed in Home Assistant for this node to function</em></p></div><h3>Configuration</h3><dl class="message-properties"><dt>Type<span class="property-type">string</span></dt><ul><li>Values <code>sensor|binary_sensor</code></li><li>Default: <code>sensor</code></li></ul><dd>The state the entity should be updated to</dd><dt>State<span class="property-type">string | number | boolean</span></dt><dd>The state the entity should be updated to</dd><dt>Attributes<span class="property-type">Object</span></dt><dd>Key/Value pair of attributes to update. The key should be a string and value can be a [string | number | boolean | object]</dd><dt>Home Assistant Config<span class="property-type">Object</span></dt><dd>Configuration options available for the selected entity</dd><dt>Input Override<span class="property-type">string</span></dt><ul><li>Values: <code>accept | merge | block</code></li></ul><dd>Determine how input values will be handled. When merge is selected the message object values will override the configuration values.</dd><dt>Resend state and attributes<span class="property-type">boolean</span></dt><dd>When creating the entity in Home Assistant this will also send the last updated state and attributes then node sent to Home Assistant</dd><div class="custom-block danger"><p class="custom-block-title">WARNING</p><p>Sensor nodes will not work in a subflow due to the way they register themselves with Home Assistant. After a Node-RED restart a new sensor will be created in Home Assistant.</p></div></dl><h3>Inputs</h3><dl class="message-properties"><dd>properties of <code>msg.payload</code></dd><dt>state<span class="property-type">string | number | boolean</span></dt><dd>The state the entity should be updated to</dd><dt>attributes<span class="property-type">Object</span></dt><dd>Key/Value pair of attributes to update. The key should be a string and value can be a <code>[string | number | boolean | object]</code></dd><dd></dd></dl></script>
